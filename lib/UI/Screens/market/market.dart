...
class _MarketScreenState extends State<MarketScreen> {
  ...
  Map<String, dynamic> fromCoin = {
    "image": "https://www.tcisscan.com/icons/tcis.png",
    "symbol": "TCIS",
    "chain": "TCIS"
  };

  Map<String, dynamic> toCoin = {
    "image": "https://www.tcisscan.com/icons/bnb.png",
    "symbol": "BNB",
  ...
  void _updateDefaultCoinsFromList() {
    final tcisCoin = listCoins.firstWhere(
          (coin) => coin['symbol'] == 'TCIS',
      orElse: () => fromCoin,
    );

    final bnbCoin = listCoins.firstWhere(
  ...
  String _formatSwapRate(double rate, String toSymbol) {
    if (rate == 0) return '0';
    final smallestUnit = _getSmallestUnit(toSymbol);
    final smallestUnitValue = _getSmallestUnitValue(toSymbol);

    if (rate < smallestUnitValue) return '~1 ${smallestUnit}';
    ...
    return formattedRate.replaceAll(RegExp(r'\.0+$'), '')
        .replaceAll(RegExp(r'\.$'), '');
  }
  ...
  Widget _buildTokenRow(String name, String imagePath, String value, String fiatValue,
    ...
    return SizedBox(
      height: isSmallScreen ? 70 : 80, // Reduced fixed height instead of percentage
      child: Stack(
        children: [
          Opacity(
