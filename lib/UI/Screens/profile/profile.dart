  ...
  String formatNumber(double value) {
    final NumberFormat format = NumberFormat.currency(
      ...
      decimalDigits: value == value.roundToDouble()
          ? 0
          : ((value * 10).roundToDouble() == value * 10) ? 1 : 2,
    );
...
class TokenInputFormDialog extends StatefulWidget {
  ...
  const TokenInputFormDialog({Key? key, required this.onAddToken, required this.walletAddress}) : super(key: key);
...
class _TokenInputFormDialogState extends State<TokenInputFormDialog> {
  ...
  bool _tokenInfoFetchedSuccessfully = false; // Cờ mới để biết thông tin token đã được lấy thành công chưa
  bool _isContractAddressFieldEnabled = true; // Cờ mới để kiểm soát trạng thái enable/disable của ô contract
  ...
  Future<void> addToken() async {
    ...
    if (!_formKey.currentState!.validate()) { // Validate lại contract address (dù đã disable)
      return;
    }

    final contractAddress = _contractAddressController.text.trim();
    // setState(() => _isFetchingTokenInfo = true); // Có thể dùng cờ khác cho isSaving

    try {
      dynamic result = await ApiService.addToken(
      ...
      Get.snackbar(
        ...
        "",
        backgroundColor: result.toString().toLowerCase().contains("success") || result.toString().toLowerCase().contains("thành công")
            ? Colors.greenAccent
            : Colors.redAccent,
        colorText: result.toString().toLowerCase().contains("success") || result.toString().toLowerCase().contains("thành công")
            ? Colors.black
            : Colors.white,
        margin: const EdgeInsets.only(top: 20, left: 16, right: 16),
        duration: const Duration(seconds: 3),
      ...
      if (result.toString().toLowerCase().contains("successful.") || result.toString().toLowerCase().contains("thành công")) {
        Future.delayed(const Duration(milliseconds: 500), () { // Delay ngắn hơn
          if (mounted) {
            widget.onAddToken();
            Navigator.pop(context);
          }
      ...
      // setState(() => _isFetchingTokenInfo = false); // Nếu dùng cờ isSaving
    }
  }
